...
myLayout = tall ||| floats ||| avoidStruts (smartBorders (Full))
...
myManageHook =
  composeAll
    [ className =? "MPlayer" --> doFloat,
      ...
      className =? "Arandr" --> doRectFloat (W.RationalRect (1 % 4) (1 % 4) (1 % 2) (1 % 2)),
      className =? "Pavucontrol" --> doCenterFloat,
      className =? "Matplotlib" --> doCenterFloat,
      resource =? "desktop_window" --> doIgnore,
      resource =? "kdesktop" --> doIgnore,
      fmap not isFloat --> insertPosition Below Newer,
      ...
      className =? "teams-for-linux" --> doShift (myWorkspaces !! 6),
  ...
  let wrapper c s
   ...
   in def
        { ppOutput = D.send dbus,
          ppCurrent = wrapper myppCurrent,
          ppVisible = wrapper myppVisible,
          ppUrgent = wrapper myppUrgent,
          ppHidden = wrapper myppHidden,
          ppTitle = wrapper myppTitle,
...
myScratchPads =
  [ NS "filemanager" spawnlf classlf sizelf,
